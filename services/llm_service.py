...
class AbstractLLMService(EventEmitter, ABC):
    ...
    async def completion(
        self, text: str, interaction_count: int, role: str = "user", name: str = "user"
    ):
        pass
        ...
        try:
            ...
            async for chunk in stream:
                ...
                if tool_calls:
                    for tool_call in tool_calls:
                        if tool_call.function and tool_call.function.name:
                            logger.info(
                                f"Function call detected: {tool_call.function.name}"
                            )
                ...
                if chunk.choices[0].finish_reason == "tool_calls":
                    ...
                    await self.emit(
                        "llmreply",
                        {"partialResponseIndex": None, "partialResponse": say},
                        interaction_count,
                    )
                    ...
                    function_response = await function_to_call(
                        self.context, function_args
                    )

                    logger.info(
                        f"Function {function_name} called with args: {function_args}"
                    )
