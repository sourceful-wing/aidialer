...
class TranscriptionService(EventEmitter):
    ...
    async def connect(self):
        ...
        await self.deepgram_live.start(
            LiveOptions(
                model="nova-2",
                language="en-GB",
                encoding="mulaw",
        ...
        self.deepgram_live.on(
            LiveTranscriptionEvents.Transcript, self.handle_transcription
        )
        ...
        self.deepgram_live.on(
            LiveTranscriptionEvents.UtteranceEnd, self.handle_utterance_end
        )
    ...
    async def handle_transcription(self, self_obj, result):
        try:
            alternatives = (
                result.channel.alternatives if hasattr(result, "channel") else []
            )
